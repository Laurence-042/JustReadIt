...
**torch 必须在 WinRT COM STA apartment 初始化之前加载其 CUDA DLL。**在同一进程内同时使用两者时，须先 `import src.ocr.manga_ocr_engine`（触发 torch 加载），再使用 `WindowsOcr`（触发 `init_apartment`）。顺序反过来会导致 `c10.dll` 初始化失败（WinError 1114）。

测试时 manga-ocr 相关测试卲在独立的 `tests/test_manga_ocr.py` 中，不与 `tests/test_ocr.py`（会初始化 WinRT）共享进程。
...
### manga-ocr 完整依赖链 License 扫描（`pip-licenses`，2026-03-02）

| 包 | 版本 | License |
|---|---|---|
| manga-ocr | 0.1.14 | Apache-2.0 |
| transformers | 5.2.0 | Apache-2.0 |
| huggingface-hub | 1.5.0 | Apache-2.0 |
| tokenizers | 0.22.2 | Apache-2.0（元数据 UNKNOWN，Hugging Face 官方确认）|
| safetensors | 0.7.0 | Apache-2.0（元数据 UNKNOWN，Hugging Face 官方确认）|
| hf-xet | 1.3.2 | Apache-2.0 |
| torch | 2.10.0 | BSD-3-Clause |
| numpy | 2.4.2 | BSD-3-Clause |
| fugashi | 1.5.2 | MIT + BSD-3-Clause |
| jaconv | 0.5.0 | MIT |
| unidic-lite | 1.0.8 | BSD-3-Clause（元数据 UNKNOWN，需手工确认）|
| filelock | 3.25.0 | MIT |
...
| Jinja2 / MarkupSafe | 3.x | BSD-3-Clause（元数据 UNKNOWN）|
| colorama | 0.4.6 | BSD-3-Clause（元数据 UNKNOWN）|
| loguru | 0.7.3 | MIT（元数据 UNKNOWN）|
| sympy / mpmath | — | BSD |
| networkx | 3.6.1 | BSD-3-Clause |

**结论**：manga-ocr 完整依赖链均为 Apache-2.0 / BSD / MIT / MPL-2.0，无 GPL / LGPL 传染性协议，与项目 MPL-2.0 完全兼容。标注 UNKNOWN 的包均为知名库，协议已通过官方文档手工确认，无风险。
...
### OCR
- [x] 集成 Windows OCR（`winrt-Windows.Media.Ocr`），实现 bounding box 返回
