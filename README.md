...
其最与众不同的地方在于，它结合了OCR的位置信息和内存扫描的精确性，使其可以根据OCR结果探测内存中潜在的原始文本，并以之修正OCR的缺字、错字，以此保证翻译的准确性

## 定位

它是什么
- 团子翻译器、WindowTranslator在特定方向的优化升级版
- 又一个以OCR为主的翻译工具，但其可以用内存扫描修正OCR结果
- 不对任何引擎做特殊适配的通用工具

它不是什么
- MTool、RenpyThief的替代品
- 翻译工具里的银弹
- 主动适配多种引擎特征的工具包
- 可以在Unity甚至UE引擎那内存占用按GiB算的游戏运行内存中找到对应文字的超级内存探针
